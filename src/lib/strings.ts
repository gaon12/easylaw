...
export const braille = {
   ...
   */
  disclaimer:
    "자동으로 바꾼 점자예요. 사람이 하나하나 확인한 결과는 아니에요. 중요한 문서는 점자 교정을 받아 주세요.",
...
export const admin = {
  ...
  judgmentHistory: "원문판 이력",
  revisionTitle: "판결문 원문판",
  revisionIntro: "원문을 다시 받을 때마다 달라진 내용을 이전 판과 비교할 수 있어요.",
  revisionBack: "판례 목록으로",
  revisionCurrent: "현재",
  revisionLegacy: "이전 시스템에서 옮긴 판",
  revisionEmpty: "저장된 원문판이 없어요.",
  revisionColumns: {
    order: "판",
    fetchedAt: "원문 받은 때",
    spans: "문장",
  ...
  revisionCompare: "직전 판과 비교",
  revisionCompareTitle: "원문판 비교",
  revisionFrom: "이전 판",
  revisionTo: "새 판",
  revisionCompareAction: "비교하기",
  revisionNoDiff: "비교할 두 원문판이 있어야 해요.",
  revisionSame: "같은 문단",
  revisionRemoved: "빠진 문단",
  revisionAdded: "추가된 문단",
  revisionOldText: "이전 원문",
  revisionNewText: "새 원문",
  releaseTitle: "쉬운 설명 공개",
  releaseIntro:
    "각 단계의 최신 설명을 원문과 대조한 뒤 게시하세요. 게시하면 현재 원문판과 묶인 새 릴리스가 남아요.",
  releaseColumns: {
    level: "단계",
    state: "상태",
    generatedAt: "최신 설명",
  ...
  releaseNoRendition: "만든 설명이 없어요.",
  releaseSentenceCount: (count: number) => `${count}문장`,
  releaseNeedsCheck: (count: number) => `확인 필요 ${count}개`,
  releaseGrounded: "근거 검사 통과",
  releaseUngrounded: (count: number) => `근거 없음 ${count}개`,
  releasePreview: "최신 설명 내용 보기",
  releasePublish: "게시",
  releaseWithdraw: "철회",
  ...
  releaseCurrent: "현재",
  releaseNoLevels: "공개 단계 없음",
  releaseHistoryColumns: {
    at: "때",
    action: "변경",
    levels: "공개 단계",
    id: "릴리스",
  ...
  releaseActions: { publish: "게시", withdraw: "철회", restore: "복원" },
  releaseRestore: "이 묶음 복원",
  releaseRestoreCurrent: "현재 공개 중인 릴리스예요.",
  releaseRestoreStale: "현재 원문판과 다른 릴리스는 복원할 수 없어요.",
  releaseActor: "관리자",
  releaseCompare: "직전 릴리스와 비교",
  releaseCompareTitle: "설명 릴리스 비교",
  releaseFrom: "이전 릴리스",
  releaseTo: "새 릴리스",
  releaseCompareAction: "비교하기",
  releaseNoDiff: "비교할 두 릴리스가 있어야 해요.",
  ...
  /** 올린 문서는 그 사람의 것이다. 무엇을 올렸는지 관리자 화면에 늘어놓지 않는다. */
  failuresUploadHidden: "올린 문서(어느 문서인지는 표시하지 않아요)",
...
export const viewer = {
  ...
  reviewingTitle: "설명을 확인하고 있어요",
  reviewingBody: "만든 설명을 원문과 대조한 뒤 공개해요. 검수가 끝나면 이곳에 나타나요.",
   ...
   * 실패한 뒤 아래 줄에 적는 말.
   *
   * **읽는 사람은 화면 앞의 이용자다.** 예전에는 파이프라인이 낸 진단을 그대로 적었는데,
   * 그 문장에는 우리가 설정한 AI API 주소와 제공자가 보낸 오류 본문이 들어 있었다.
   * 아무나 여는 판결문 페이지에 운영 설정이 찍혔고, 정작 그걸로 고칠 수 있는 관리자는
   * 그 화면을 보지 않는다 — 아무도 이득을 보지 못했다.
   *
   * "숨기면 고칠 수도 없다"는 판단 자체는 옳았다. **보여 줄 곳을 틀렸을 뿐이다.**
   * 진짜 원인은 `/admin`의 "최근 실패"로 간다.
   *
   * 여기 있는 문장은 전부 **읽는 사람이 할 수 있는 일**로 끝난다 — 다시 눌러 볼지,
   * 기다릴지, 관리자에게 알릴지, 아니면 원문만 볼지.
   */
  failedReasons: {
    /** AI 연결이 아예 없다. */
    notConfigured: "설명 만들기가 아직 준비되지 않았어요. 원문은 그대로 보실 수 있어요.",
    /** 원문을 못 읽었다. 설명 이전의 문제다. */
    noOriginal: "원문을 가져오지 못해 설명을 만들 수 없었어요.",
    /** 구조를 하나도 못 뽑았다. 이 판결문 자체가 짧거나 형태가 다른 경우다. */
    noStructure: "이 판결문에서는 설명할 내용을 찾지 못했어요.",
    /**
     * 근거를 원문에 붙이지 못한 문장이 남았다(P2). **만들다 만 것을 보여 주지 않는다.**
     * 그래서 이 문장은 사과가 아니라 "원문은 있다"는 안내로 끝난다.
     */
    ungrounded:
      "원문에서 근거를 확인할 수 있는 설명을 만들지 못했어요. 원문은 그대로 보실 수 있어요.",
    /** 타임아웃·과부하처럼 다시 걸면 될 법한 것. */
    busy: "지금은 AI 서버가 답하지 않아요. 잠시 뒤에 다시 눌러 주세요.",
    /** 키·주소·모델 이름이 틀렸다. 이용자가 할 수 있는 일이 없으므로 알릴 곳을 알려 준다. */
    misconfigured: "설명 만들기 설정에 문제가 있어요. 이 사이트 관리자에게 알려 주세요.",
    /** 모델이 규격에 맞지 않게 답했다. 다시 하면 될 때가 많다. */
    modelOutput: "설명을 만들다 실패했어요. 다시 눌러 보시면 될 때가 많아요.",
    /** 그 밖. */
    unknown: "설명을 만들다 문제가 생겼어요. 잠시 뒤에 다시 눌러 주세요.",
  },
