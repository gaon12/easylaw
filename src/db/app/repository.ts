...
import { auditLog, upload, uploadMask, uploadSpan, user } from "./schema";
...
function findUserByOwnerKeyHash(db: AppDb, ownerKeyHash: string) {
  return db.select().from(user).where(eq(user.ownerKeyHash, ownerKeyHash)).get();
}

/** 익명 소유자를 만든다. 토큰 원문이 아니라 해시만 받는다. */
function createUser(db: AppDb, ownerKeyHash: string): string {
  const id = newId();
  db.insert(user).values({ id, ownerKeyHash, lastSeenAt: new Date() }).run();
  return id;
}
