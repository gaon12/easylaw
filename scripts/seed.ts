 ...
 *
 * **잘라 두지 않는다.** 한때 파서 테스트용으로 앞부분만 떠 놨는데, 시드가 같은 파일을
 * 읽는 바람에 화면에 문장 도중에서 끊긴 판결문이 올라갔다(마지막 문단이 "…라 한다)는 20"
 * 에서 끝나고, 뒤에 붙은 주문이 앞의 주문과 어긋났다). 파서를 시험하는 데는 상관없어도
 * **이 파일은 화면에도 올라간다** — 응답 전체를 그대로 둔다.
 *
